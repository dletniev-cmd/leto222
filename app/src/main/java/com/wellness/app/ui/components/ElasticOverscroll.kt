...
internal class ElasticOverscrollConnection(
    ...
    override suspend fun onPostFling(
        ...
        // Safety: in case the inner scroll fling ended with us still in
        // overscroll (e.g. fling kept dragging past edge), settle to 0.
        if (verticalOverscroll.floatValue != 0f && verticalJob?.isActive != true) {
            verticalJob = scope.launch {
                springBack(verticalOverscroll, 0f, maxVerticalPx)
            }
        }
        if (horizontalOverscroll.floatValue != 0f && horizontalJob?.isActive != true) {
            horizontalJob = scope.launch {
                springBack(horizontalOverscroll, 0f, maxHorizontalPx)
            }
        }
    ...
    private suspend fun springBack(
        ...
        val dampedVelocity = initialVelocity * velocityScale
