...
fun TextInput(
    ...
    Box(
        ...
        if (value.isEmpty()) {
            Text(placeholder, color = Wellness.colors.muted, style = Wellness.typography.bodyMedium)
        }
        BasicTextField(
            value = value,
    ...
    NoFeedbackButton(onClick = onClick, modifier = modifier) {
        Box(
            modifier
                ...
                .height(56.dp)
                .background(
                    if (active) tint.copy(alpha = 0.16f) else Wellness.colors.track,
                    RoundedCornerShape(14.dp),
                ),
            contentAlignment = Alignment.Center,
        ) {
            SolarIcon(name = icon, tint = if (active) tint else Wellness.colors.text, size = 22.dp)
        }
    }
}
